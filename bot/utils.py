...
async def soft_scan_channel(channel, link_db):
    ...
    logger.info(f"Starting soft scan for channel {channel.id}...")
    ...
    async for message in channel.history(limit=None, oldest_first=False):
        ...
        if latest_tracked_id and message.id <= latest_tracked_id:
            logger.info(f"Reached already tracked message {latest_tracked_id}, soft scan complete")
            break
        ...
        if message.content and message.content.startswith("https"):
            ...
            if video_id:
                ...
                if not existing:
                    ...
                    if success:
                        new_links += 1
                        logger.info(f"Soft scan found new link: {video_id} from {message.author}")

    logger.info(f"Soft scan complete: scanned {scanned_count} messages, found {new_links} new links")
    return new_links

async def check_deleted_messages_fast(channel, link_db, message_cache):
    """Ultra-fast deletion check using message cache"""
    logger.info(f"Fast deletion check for channel {channel.id}...")
    ...
    if not active_messages:
        logger.info("No active messages to check in cache range")
        return 0

    logger.info(f"Checking {len(active_messages)} messages against cache...")
    deleted_count = 0
    ...
    for message_id, video_id, author_name in active_messages:
        if message_id not in message_cache:
            ...
            deleted_count += 1
            logger.info(f"Marked message {message_id} (video: {video_id}) as deleted")

    logger.info(f"Fast deletion check complete: marked {deleted_count} messages as deleted")
...
async def check_link_realtime(message, link_db):
    ...
    if existing:
        ...
        if success:
            logger.info(f"New link logged to DB: {video_id} from {message.author}")
        return None